...
!!				write(*,*) 'scat ',n,ivv,ivvPre,kx,kxPre,ky,kyPre,kz,kzPre,eee,eeeCalc,eeePre,eeePreCalc	!	debug
!!				write(8,*) 'scat ',n,ivv,ivvPre,kx,kxPre,ky,kyPre,kz,kzPre,eee,eeeCalc,eeePre,eeePreCalc	!	debug
!
!!			if (tdt > dt*float(jtl)) then										!	debug
!!			if (tdt <= dt) then													!	debug
!!				write(*,*) 'scat ',ivv,ivvPre,kx,kxPre,ky,kyPre,kz,kzPre,eee,eeePre	!	debug
!!				write(8,*) 'scat ',ivv,ivvPre,kx,kxPre,ky,kyPre,kz,kzPre,eee,eeePre	!	debug
!!			end if																!	debug
...
!!	eee=2.d0*gk/(sqrt(1.d0+4.d0*alpha*gk)+1.d0)				!	不要な桁落ち防止？2018.11.15
!
...
!!		!	trial
	...
	do while(REDO > 0)
		...
		sphi = sin(phi) 
		cphi = cos(phi) 
		kz = (cbet*cgam   +sbet*cphi*sgam)*sqrt(mz(ivv)/mdos)*pprime/hbar
		ky = (cbet*cgamp  -sbet*cphi*cgam*ceta -sbet*sphi*seta)*sqrt(my(ivv)/mdos)*pprime/hbar
		kx = (cbet*cgampp -sbet*cphi*cgam*seta +sbet*sphi*ceta)*sqrt(mx(ivv)/mdos)*pprime/hbar
		gk=0.5d0*hbar*hbar*(kx*kx/mx(ivv)+ky*ky/my(ivv)+kz*kz/mz(ivv))
