...
!		from Tom_global_variables   !----- 共通変数 -----
	...
	Double Precision de
	Double Precision :: swk(18,4002)=0.d0
...
!		--- phonon emission
		...
		if (ef > 0.0) then
			...
			ph_qmax = sei+sef
			ph_qmin = sei-sef
			swk(6,ie)= poe*sqrt(2.0*mdos)*sei*log(ph_qmax/ph_qmin)/(hbar*ei)
		else
			swk(6,ie)= 0.0
		end if
!		--- phonon absorption
		...
		ph_qmax = sei+sef
		ph_qmin = -sei+sef
		swk(7,ie)= poa*sqrt(2.0*mdos)*sei*log(ph_qmax/ph_qmin)/(hbar*ei)
!
...
!			eee = de*float(ie-1)
!		end if
!		write (*,'(18E22.15)') eee,swk(2,ie),swk(3,ie),swk(4,ie),swk(5,ie),swk(6,ie),swk(7,ie),swk(18,ie)
!		write (8,'(18E22.15)') eee,swk(2,ie),swk(3,ie),swk(4,ie),swk(5,ie),swk(6,ie),swk(7,ie),swk(18,ie)
!	end do
...
!
		...
		call final_state_impurity_scat(kx,ky,kz,eee)			! Global kx, ky, kz
	end if
!      
end subroutine scat
!========================================================================================
